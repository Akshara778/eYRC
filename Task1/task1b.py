...
class FruitsTF(Node):
    ...
    def __init__(self):
        ...
        self.publisher = TransformBroadcaster(self, 10)
        self.buffer = Buffer()
    ...
    def bad_fruit_detection(self, rgb_image):
        ...
        img_hsv_inrange = cv2.inRange(img_hsv, lower_bound, upper_bound)

        # Step 4: Find contours in the mask
        #   - Use cv2.findContours to detect continuous regions (potential bad fruits)
        img_contours, _ = cv2.findContours(img_hsv_inrange, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        ...
        for contour in img_contours:
            if cv2.contourArea(contour) < 1800 or cv2.contourArea(contour) > 4200:
                continue
            ...
            angle = 0
            fruit_info = {'center': (cX, cY), 'distance': distance, 'angle': angle, 'width': w, 'height': h, 'id': fruit_id}
    ...
    def process_image(self):
        ...
        for fruit in bad_fruits:
            ...
            cv2.rectangle(self.cv_image, (int(cX - w/2), int(cY - h/2)), (int(cX + w/2), int(cY + h/2)), (0, 255, 0), 2)
            cv2.putText(self.cv_image, "bad fruit", (int(cX - w/2), int(cY - h/2 - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            tf = TransformStamped()
            tf.header.frame_id = 'camera_link'
            tf.child_frame_id = f'cam_{id}'
            tf.transform.translation.x = z
            tf.transform.translation.y = x
            tf.transform.translation.z = y


            self.publisher.sendTransform(tf)

            while not self.buffer.can_transform('base_link', f'cam_{id}', Time()):
                rclpy.spin_once(self)

            transform = self.buffer.lookup_transform('base_link', f'cam_{id}', Time())

            tf_base = TransformStamped()
            tf_base.header.frame_id = 'base_link'
            tf_base.child_frame_id = f'4558_bad_fruit_{id}'
            ...
            self.publisher.sendTransform(tf_base)

        #   ->  At last show cv2 image window having detected markers drawn and center points located using 'cv2.imshow' function.
        #       Refer MD book on portal for sample image -> https://portal.e-yantra.org/

        cv2.imshow('fruits_tf_view', self.cv_image)
        key = cv2.waitKey(1)
