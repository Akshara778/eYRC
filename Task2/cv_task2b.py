...
class aruco_tf(Node):
    ...
    def process_image(self):
        ...
        for i in range(len(id_list)):
            ...
            if not self.tf_buffer.can_transform('base_link', 'cam1_' + str(id_list[i]), rclpy.time.Time()):
                return
            ...
            tf_base.child_frame_id = 'obj_' + str(id_list[i])
            tf_base.header.stamp = self.get_clock().now().to_msg()
            if id_list[i] == 3:
                tf_base.child_frame_id = '4558_fertilizer_1'
